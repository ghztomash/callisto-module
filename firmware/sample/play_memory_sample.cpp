...
void AudioPlayMemorySample::update(void)
{
	...
	int i;
	uint32_t index;
	int64_t local_phase_increment, scale;
			...
			if (phase + local_phase_increment> sampleSize) {
					...
					*out++ = 0;
					...
					if(interpolate){
						...
						scale = (phase >> 31);
						tmp32a = tmp32a * scale;
						tmp32b = tmp32b * (0x10000 - scale);
						
						*out++ = (int16_t)((tmp32a + tmp32b) & 65535);
						*out++ = (int16_t)((tmp32a + tmp32b) >> 16);
					}
					else {
						tmp32a = in[index];
						*out++ = (int16_t)(tmp32a & 65535);
						*out++ = (int16_t)(tmp32a >> 16);
					}
			}
				//Serial.println(*pmod);
